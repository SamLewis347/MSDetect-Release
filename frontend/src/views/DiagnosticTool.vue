<template>
  <div class="relative min-h-screen text-slate-100 flex flex-col items-center py-16 px-6">
    <!-- Header -->
    <div class="mt-10 w-full max-w-7xl mb-10 flex justify-between items-center">
      <div>
        <h1 class="text-4xl font-extrabold"><span class="text-white">MS</span><span class="text-sky-400">Detect</span> Portal</h1>
        <p class="text-slate-400 text-sm mt-1">Upload and analyze MRI scans using AI-assisted segmentation.</p>
      </div>
    </div>

    <!-- Unified Workspace -->
    <div class="w-full max-w-7xl bg-slate-950/80 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row">
      <!-- Upload Section -->
      <div class="lg:w-1/3 p-8 border-b lg:border-b-0 lg:border-r border-white/10 flex flex-col justify-between">
        <div>
          <h2 class="text-2xl font-semibold mb-2 text-white">Upload MRI</h2>
          <p class="text-slate-400 mb-5 text-sm">Click or drag your MRI (.nii / .nii.gz) here for AI preview and prediction.</p>
          ...
          <div
            ...
            :class="[
              'rounded-xl p-6 w-full flex flex-col items-center justify-center text-center cursor-pointer transition-all border-2 border-dashed',
          ...
          >
            ...
            <i class="fa-solid fa-cloud-arrow-up text-sky-400 text-4xl mb-3"></i>
            <p class="text-white font-medium">Click or drag to upload</p>
            <p class="text-slate-400 text-xs mt-1">Accepted: .nii, .nii.gz</p>
          </div>

          <!-- File Info -->
          <div v-if="fileName" class="mt-4 text-sm text-slate-300">
            <div>File: <span class="text-sky-300">{{ fileName }}</span></div>
          </div>

          <!-- Preview Loading State -->
          <div v-if="loadingPreview" class="mt-4 flex items-center gap-2 text-sm text-sky-400">
        ...
        <!-- Actions -->
        <div v-if="fileName && !loadingPreview" class="flex items-center gap-3 mt-6">
          <button
            class="bg-gradient-to-r from-sky-600 to-sky-500 px-4 py-2 rounded-md text-white hover:scale-[1.02] transition-all shadow disabled:opacity-50 disabled:cursor-not-allowed"
            @click="submitFile"
            :disabled="loading"
          >
          ...
          <button
            class="px-3 py-2 rounded-md border border-white/10 text-sm text-slate-200 hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <!-- Viewer Section -->
      <div class="lg:w-2/3 p-8 relative flex flex-col min-h-[600px]">
        <!-- Bezel Header -->
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="text-xs text-slate-400">
            Patient: <span class="text-white">Demo</span> • Study:
        ...
        <!-- MRI Viewer -->
        <div
          class="relative flex-grow border border-white/10 rounded-2xl bg-slate-950 flex items-center justify-center overflow-hidden shadow-inner min-h-[380px]"
          @wheel.prevent="handleScroll"
        >
          <!-- Loading States -->
          <div v-if="loadingPreview" class="flex flex-col items-center gap-4">
            <div class="w-16 h-16 border-4 border-sky-400 border-t-transparent rounded-full animate-spin"></div>
            <div class="text-slate-400 text-sm">Processing MRI volume...</div>
            <div class="text-slate-500 text-xs">Extracting axial slices</div>
          </div>

          <div v-else-if="loading" class="flex flex-col items-center gap-4">
            <div class="w-16 h-16 border-4 border-sky-400 border-t-transparent rounded-full animate-spin"></div>
            <div class="text-slate-400 text-sm">Running AI analysis...</div>
            <div class="text-slate-500 text-xs">This may take a moment</div>
            <!-- Progress bar -->
            <div class="w-64 h-2 bg-slate-800 rounded-full overflow-hidden mt-2">
          ...
          <img
            ...
            class="object-contain max-h-[75vh] scale-[1.5] transition-transform duration-300 ease-out"
          />
          <div v-else class="text-slate-500/70 text-sm">No scan loaded — upload to preview.</div>
        ...
        <div class="flex items-center justify-between mt-4">
          <div class="flex items-center gap-3">
            <button
              class="px-3 py-2 bg-slate-800/60 border border-white/10 rounded-md text-sm hover:bg-slate-700/60 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <button
              class="px-3 py-2 bg-sky-600/80 text-white rounded-md text-sm shadow hover:bg-sky-500/80 disabled:opacity-50 disabled:cursor-not-allowed"
              @click="toggleHeatmap"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-6">
          <div class="col-span-2 bg-slate-900/60 border border-white/10 rounded-lg p-3">
            <div class="text-xs text-slate-400">AI Findings</div>
            <div v-if="loading" class="mt-2">
              <div class="text-lg font-bold text-amber-400">Processing...</div>
              <div class="text-sm text-slate-300 mt-1">
                Running inference on MRI volume
              </div>
            </div>
            <div v-else-if="predictionSlices.length > 0" class="mt-2">
              <div class="text-lg font-bold text-sky-300">Analysis Complete</div>
              <div class="text-sm text-slate-300 mt-1">
            ...
            <div v-else class="text-sm text-slate-400 mt-2">No prediction yet — upload & run analysis.</div>
          </div>
          ...
          <div class="bg-slate-900/60 border border-white/10 rounded-lg p-3">
            ...
            <div class="mt-2 text-sm text-slate-200">
              <div>Filename: <span class="text-slate-300">{{ fileName || "—" }}</span></div>
        ...
        <div class="text-xs text-slate-500 mt-3 border-t border-white/5 pt-3">
